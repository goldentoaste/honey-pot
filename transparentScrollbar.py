...
class TransScrollBar(QWidget):
    def __init__(self, orient: Qt.Orientation, parent: QWidget, scrollArea: QScrollArea) -> None:
        ...
        parent.enterEvent = (lambda originalEnter: (lambda event: (originalEnter(event), self.parentEntered(event))))(
            parent.enterEvent
        )
        parent.leaveEvent = (lambda originalLeave: (lambda event: (originalLeave(event), self.parentLeave(event))))(
            parent.leaveEvent
        )
    ...
    def configChanged(self, config: Config):
        ...
        self.minBarSize = config.iScrollbarMinSize
    ...
    def updateValue(self):
        self.value = int(
            (self.proxy.value() / (self.range)) * ((self.height() if self.isVert else self.width()) - self.stepSize)
        )
    ...
    def barChanged(self, change: QScrollBar.SliderChange):
        ...
        if not self.clicked and change == QScrollBar.SliderChange.SliderValueChange:
            ...
            self.stepSize = max(
                int((self.proxy.pageStep() / self.range) * (self.height() if self.isVert else self.width())),
                self.minBarSize,
            )
    ...
    def sizeHint(self):
        ...
        return QSize(self.parent().width() - self.barWidth, self.barWidth)  # hor bar should always yield to vert bar
