    ...
    def insertFromMimeData(self, source: QMimeData) -> None:
        ...
        if source.hasUrls(): # prob a local file
            ...
            for url in urls:
                ...
                if url.isLocalFile():# BUG just checking for isLocalFile() aka "file://" header is not enough, could be a network file
                    imagePath = url.path().strip("/").strip("\\")
                    print(f"parsed uri path: {imagePath}")
                    if not imagePath.endswith(('.jpg', '.png', '.bmp', '.jpeg')): return # ignore non image files
